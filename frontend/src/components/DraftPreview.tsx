...
import { X, Mail, FileText, Clock, Zap, Hash } from "lucide-react";
        ...
        >
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b border-border">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-semibold">Job Detail</span>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              <span className={`text-xs font-medium px-2 py-1 rounded-md ${
                ...
                job.status === "running" ? "bg-primary/15 text-primary-light" :
                job.status === "skipped" ? "bg-warning/15 text-warning" :
                "bg-surface-2 text-muted"
              }`}>
                {job.status}
              </span>
              {job.intent && (
                <span className="text-xs font-medium px-2 py-1 rounded-md bg-surface-2 text-muted">
                  Intent: {job.intent}
                </span>
              )}
              {job.confidence != null && (
                <span className="text-xs font-mono px-2 py-1 rounded-md bg-surface-2 text-muted">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="p-3 rounded-lg bg-surface-2">
                ...
                <span className="text-sm font-mono">{job.tokens_used}</span>
              </div>
              <div className="p-3 rounded-lg bg-surface-2">
                <div className="flex items-center gap-1.5 mb-1">
                ...
                <span className="text-sm font-mono">{job.duration_ms ? `${(job.duration_ms / 1000).toFixed(1)}s` : "—"}</span>
              </div>
            </div>
            ...
            {job.summary && (
              <div>
                ...
                <p className="text-sm mt-1 p-3 rounded-lg bg-surface-2 leading-relaxed">
                  {job.summary}
                </p>
              </div>
            )}
            ...
            {job.error_msg && (
              <div>
                ...
                <pre className="text-xs mt-1 p-3 rounded-lg bg-error/10 text-error font-mono overflow-x-auto whitespace-pre-wrap">
                  {job.error_msg}
                </pre>
              </div>
            )}
            ...
            {job.draft_id && (
              <div>
                ...
                <p className="text-xs mt-1 p-2 rounded bg-surface-2 font-mono text-muted break-all">
                  {job.draft_id}
                </p>
              </div>
            )}
